...
int crearSala(TListaSala *lista, char nombre_origen[20], int idSocket){
	...
	if(encontrado == 0){
		...
		return 0; //si se a creado la sala
	}else{
...
void *AtenderCliente (void *socket){
	...
	while(terminar == 0){
		...
		//aqui reenviamos el dibujo a todos los jugadores de la sala
			...
			int j = 0;
			printf("Nombre: %s", nombre_jugador);
			printf("Posicion: %d", i);
			printf("NumActual: %d", listaSalas.salas[i].numActual);
			while(j<2){
				printf("Socket: %d\n",listaSalas.salas[i].socketsJugadores[j]);
				write(listaSalas.salas[0].socketsJugadores[j], respuesta, strlen(respuesta));
				printf("Respuesta: %s\n", respuesta);
				j = j+1;
			}
		}
		...
		//aqui el servidor cuando recibe esta peticion generara una palabra aleatoria y la reenviara, el usuario al que reenvia lo recibe como parametro
			...
			if( listaSalas.salas[i].ronda >= 0){
				
				
				int socketElegido = listaSalas.salas[i].socketsJugadores[listaSalas.salas[i].ronda];
